...
void
var_list_export_c(VAR *vl, const char *filename)
{
    FILE *fc, *fh;
    char fname_c[256], fname_h[256];
    char varname[256];
    int i, j, nn;
    char *tok;
    double *f8a;
    ...
    snprintf(fname_c, sizeof(fname_c), "%s.c", filename);
    snprintf(fname_h, sizeof(fname_h), "%s.h", filename);

    fc = fopen(fname_c, "w");
    fh = fopen(fname_h, "w");
    if (!fc || !fh) {
        printf("Error: Could not open export files %s.c / .h\n", filename);
        if (fc) fclose(fc);
        if (fh) fclose(fh);
        return;
    }

    fprintf(fh, "// Generated by BTMHA export\n");
    fprintf(fh, "#ifndef BTMHA_EXPORT_H\n#define BTMHA_EXPORT_H\n\n");
    fprintf(fh, "#include <stdint.h>\n\n");

    fprintf(fc, "// Generated by BTMHA export\n");
    fprintf(fc, "#include \"%s\"\n\n", fname_h);

    for (i = 0; i < MAX_VAR; i++) {
        if (!vl[i].name || !vl[i].data) continue;

        strncpy(varname, vl[i].name, sizeof(varname)-1);
        varname[sizeof(varname)-1] = '\0';
        for (j = 0; varname[j]; j++) {
            if (varname[j] == '.') varname[j] = '_';
        }

        nn = vl[i].rows * vl[i].cols;

        if (vl[i].dtyp == SP_DTYP_F8) {
            f8a = (double *)vl[i].data;
            if (nn > 1) {
                fprintf(fh, "extern const float %s[%d];\n", varname, nn);
                fprintf(fc, "const float %s[%d] = {\n    ", varname, nn);
                for (j = 0; j < nn; j++) {
                    fprintf(fc, "%.7ef%s", f8a[j], (j < nn - 1) ? ", " : "");
                    if ((j + 1) % 5 == 0 && j < nn - 1) fprintf(fc, "\n    ");
                }
                fprintf(fc, "\n};\n\n");
            } else {
                fprintf(fh, "extern const float %s;\n", varname);
                fprintf(fc, "const float %s = %.7ef;\n\n", varname, f8a[0]);
            }
        } else if (vl[i].dtyp == SP_DTYP_I4) {
            i4a = (int32_t *)vl[i].data;
            if (nn > 1) {
                fprintf(fh, "extern const int32_t %s[%d];\n", varname, nn);
                fprintf(fc, "const int32_t %s[%d] = {\n    ", varname, nn);
                for (j = 0; j < nn; j++) {
                    fprintf(fc, "%d%s", i4a[j], (j < nn - 1) ? ", " : "");
                    if ((j + 1) % 10 == 0 && j < nn - 1) fprintf(fc, "\n    ");
                }
                fprintf(fc, "\n};\n\n");
            } else {
                fprintf(fh, "extern const int32_t %s;\n", varname);
                fprintf(fc, "const int32_t %s = %d;\n\n", varname, i4a[0]);
            }
        } else if (vl[i].dtyp == SP_DTYP_U1) {
            tok = (char *)vl[i].data;
            if (vl[i].cols > 1) {
                fprintf(fh, "extern const char * const %s[%d];\n", varname, vl[i].cols);
                fprintf(fc, "const char * const %s[%d] = {\n    ", varname, vl[i].cols);
                for (j = 0; j < vl[i].cols; j++) {
                    fprintf(fc, "\"%s\"%s", tok + j * vl[i].rows, (j < vl[i].cols - 1) ? ", " : "");
                }
                fprintf(fc, "\n};\n\n");
            } else {
                fprintf(fh, "extern const char * const %s;\n", varname);
                fprintf(fc, "const char * const %s = \"%s\";\n\n", varname, tok);
            }
        }
    }

    fprintf(fh, "#endif /* BTMHA_EXPORT_H */\n");

    fclose(fc);
    fclose(fh);
    printf("Exported C code to %s.c and %s.h\n", filename, filename);
}
